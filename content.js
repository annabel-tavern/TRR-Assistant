// ============================================================
// RealReal Enhancer - Content Script (v2.0)
// Toolbar with filter presets, compact view, dim sold items.
// Item labeling is in content-labels.js.
// ============================================================
...
(function () {
  ...
  function attachEvents(tb) {
    ...
    tb.querySelectorAll('.trr-preset-btn').forEach((b) => b.addEventListener('click', (e) => {
      e.preventDefault(); const p = presets.find((x) => x.id === b.dataset.pid); if (p) navPreset(p, e.shiftKey);
    }));
    tb.querySelectorAll('.trr-pa-edit').forEach((b) => b.addEventListener('click', (e) => {
      e.stopPropagation(); const p = presets.find((x) => x.id === b.dataset.pid); if (p) showEditModal(p);
    }));
    tb.querySelectorAll('.trr-pa-del').forEach((b) => b.addEventListener('click', (e) => {
      e.stopPropagation(); const p = presets.find((x) => x.id === b.dataset.pid); if (p) showDeleteModal(p);
    }));
    tb.querySelectorAll('.trr-preset-wrap').forEach((w) => w.addEventListener('contextmenu', (e) => {
      e.preventDefault(); const p = presets.find((x) => x.id === w.dataset.pid); if (p) showDeleteModal(p);
    }));
    ...
    tb.querySelectorAll('.trr-emoji-btn').forEach((b) => b.addEventListener('click', () => {
      tb.querySelectorAll('.trr-emoji-btn').forEach((x) => x.classList.remove('trr-es')); b.classList.add('trr-es'); selEmoji = b.dataset.emoji;
    }));
  ...
  async function handleSave() {
    ...
    const emoji = tb._emoji(), desc = document.getElementById('trr-inp-desc').value.trim();
    const parsed = parseUrl(location.href);
    if (editingPresetId) {
      const i = presets.findIndex((p) => p.id === editingPresetId); if (i === -1) { closeModal(); return; }
      ...
      await savePresets(); closeModal(); refreshToolbar(); showToast(`✓ Saved "${name}"`);
    }
  }
